...
def readPotentialFile(path_to_potfile):
    ...
    p_val = readValuesFromLine(read_file_chars[5])
    q_val = readValuesFromLine(read_file_chars[6])
    a_val = readValuesFromLine(read_file_chars[7])
    qsi_val = readValuesFromLine(read_file_chars[8])
    cohe_val = readValuesFromLine(read_file_chars[11])
    atom_rad_val = readValuesFromLine(read_file_chars[12])
    mass_val = readValuesFromLine(read_file_chars[13])
    cutoff_val = readValuesFromLine(read_file_chars[16])
...
def pressureMain():
    ...
    NATOM = int(movie.Headers[0][2])+int(movie.Headers[0][4])
    ...
    for frame_num, current_frame in enumerate(movie.Frames):
        ...
        for i in range(len(current_frame)): # Loop over i
            ...
            pressure_bond_i = 0.0
            summed_denom_i = 0.0
            atomic_volume = (4./3.)*np.pi*potential.Radius[0]**3
